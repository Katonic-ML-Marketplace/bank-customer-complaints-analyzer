...
NAVBAR = dbc.Navbar(
    children=[
        ...
        html.A(
            ...
            dbc.Row(
                [
                    dbc.Col(html.Img(src=app.get_asset_url("logo.png"), style={"float": "right", "height": 40})),
                    dbc.Col(
                        dbc.NavbarBrand("Bank Customer Complaints Analyzer", className="ml-2")
                    ),
                ],
                align="center"
               # no_gutters=True,
...
BODY = dcc.Tabs(id = "tabs-02",
   children =  [
        dcc.Tab(label="Comparison",children=[dbc.Row([dbc.Col(dbc.Card(TOP_BIGRAM_COMPS)),], style={"marginTop": 30})],),
        dcc.Tab(label="Bigrams",children=[dbc.Row([dbc.Col(dbc.Card(TOP_BIGRAM_PLOT)),], style={"marginTop": 30}),],),
        dcc.Tab(label="Chart",children=[dbc.Row(
            [
                dbc.Col(jumbotron , md=4, align="center"),
                dbc.Col(dbc.Card(TOP_BANKS_PLOT), md=8),
            ],
            style={"marginTop": 30},
        )],),
        dcc.Tab(label="Frequently Used Words",children=[dbc.Row(WORDCLOUD_PLOTS)],),
        dcc.Tab(label="Topic Analysis",children=[dbc.Row([dbc.Col([dbc.Card(LDA_PLOTS)])], style={"marginTop": 50})],),
    ],
    className="mt-12",
)
...
app.title = "Bank Customer Complaints Analyzer"
